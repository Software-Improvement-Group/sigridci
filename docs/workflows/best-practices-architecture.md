...
Software architecture has a central position when it comes to and managing the life cycle of large-scale software landscapes. A key part of a healthy software architecture is designing a healthy and future-proof component architecture. By incorporating functional and technical aspects in reusable blocks, component architectures allow for higher-level understanding of the software landscape as a whole. This has many benefits, but is in particular useful when considering system-wide concerns such as system evolution. However, with the ever-growing complexity and the move towards distributed and service-oriented software systems, this is becoming an increasingly challenging task to achieve.

Tracking your architecture evolution is therefore key to ensuring your software landscapes and its systems remain future-proof. Every architecture is bound to change over time, due to a changing environment in which it operates. These guidelines are meant to help architects and development teams with their component architecture, based on best practices.

<sig-toc></sig-toc>

## Why modularize your codebase?

Every non-trivial system is split into components. However, there are multiple different reasons for why people decide to modularize their system. Some of these reasons strictly relate to technical benefits, and other reasons purely relate to making life easier for the people working on the system. 
...
Many systems use a combination of these reasons when designing their component architecture. For example, it is quite common to define the top-level architecture as a number of microservices that are deployed separately. But every microservice also has an internal architecture, which is mainly based on dividing the codebase into logical areas. Such a combination is not “wrong.” It is a perfectly reasonable choice which indicates that modularizing a system needs to consider both technical and human aspects.
...
Defining components in this way is perfectly fine. Using a reference modularization pattern also has a major benefit: Anyone who knows the pattern will immediately understand the modularization of your system. People will understand where new functionality should be implemented, since every layer has clearly defined boundaries. 
...
This effectively adds a level of abstraction. You still get the clearly defined roles of the controllers, services, and repositories. But this adds a functional structure on top of that technical structure. This allows you to track and manage dependencies between functional components. Also, since most changes relate to functional area, this means that changes have a more clearly defined scope. 

When it comes to defining components, consistency can be a double-edged sword. Defining functional components has benefits, but it’s almost impossible to define such a structure for your entire system. You’re almost inevitably going to end up with a “common” or “shared” or “utility” component that contains cross-cutting concerns used by all functional components. The outcome is a mixed structure, where most of your components are defined based on functional boundaries, but you also have a small number of technical components. This is fine, having 100% consistency across all components is not a goal in itself.
...
The initial component structure is usually designed at the beginning of the project, often before the first line of code has been written. A common pitfall is to basically retain this initial structure forever, and never revisit it. Over the years, systems tend to involve, and the original component structure is not necessarily designed to handle those changes. 

A suitable component structure tends to look like a tree. Defining 100 top-level components is not particularly helpful for locating functionality, so even very large systems should strive towards a manageable number of top-level components. However, larger systems tend to have more levels of sub-components. Every component has its own internal structure, and as components continue to grow, they tend to spawn additional levels of sub-components to manage their internal structure. 
