...
_not sure what the source of this is: can these all be moved to specific JTBDs?_
...
***not yet consistent with the guidelines list!!***
1. *Define OSH related policies*
1. Implement usage of a package manager
1. Scan the software for issues
1. Updating/adopting/introducing/modifying a library
1. Handling detected vulnerabilities
1. Handling detected license issues
1. Handling detected lack of freshness
...
### OSH Sigrid-specific Jobs to be done
  9. *Setting up Sigrid OSH*
...
> relate this to the guidance for producers! 

### Define OSH related policies
...
### Updating/Adopting/introducing/modifying a library
- checklist:
...
### Bug detected in dependency:
- Fixed in new version? -> Consider updating
- ​If not: Writing own bug fix? --> Merge back into library (check license) -> When community accepted fix, remove the local code​
  - Or: Wait on new version -> and Check new version for bugfix 

### Scan/check a library before adopting it [how can Sigrid help?]
...
- when is postponing updates a good idea?
  ...
  - For core functionalities, it is a good idea to stay on a stable version of the library.​Being an early adapter of a new version comes with some risks. Bugs are common in immature versions of libraries as they are not thoroughly tested. Although you have the opportunity to contribute, by testing and potentially providing solutions to bugs you encounter in open source libraries, it might be a better practice to wait until the library becomes more mature.​​
  - Don’t go for the ”If it ain’t broke, don’t fix it” strategy​
    - This strategy implies that you don’t update unless you have to. You stay with the current version of the third-party dependency until you notice something wrong in your application, no matter how often the vendor publishes an update. ​
