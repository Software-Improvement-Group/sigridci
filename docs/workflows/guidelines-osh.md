...
Package managers need to be integrated in your CI/CD pipeline.
...
<!-- >  or reverse the order of _when_, and _how_? -->
...
- If the vulnerability does not pose any actual risk, ‘allowlist’ it: that means  allowing the specific vulnerability for this library/application to be present. This requires CISO approval. This _allowlist_ will be reviewed as part of a half-yearly measurement cycle. 
  <!-- we already have a 'permitted-list' for checked and permitted libraries, and an 'ignore-list' for libraries that are not checked; is this a third category? -> I think OSH findings refinement should solve this dilemma? -->
   <!-- > TODO: add how to do this in Sigrid? -->
<!-- later: refer to explanations when they are added to the OSH -->

- The risk can be mitigated in some other way. If, for example, the vulnerability is limited to a single method in the library that is not called by your application. You can then test for the use of that method and fail the pipeline in that case, to prevent future accidental risks. 
  
- You can replace, or stop using, the library completely; see [12. Replacing a library](#12-replacing-a-library) for more details.
...
---
> COMMENT: here's an experiment with a graphical overview of the process: please provide feedback whether such an overview is helpful! (NB: to be viewed on Github)
```mermaid
graph TD; 
...
One possible approacj in this case can be to wrap the new library and in this way provide an interface that is equal, or more similar, to the previous library.
