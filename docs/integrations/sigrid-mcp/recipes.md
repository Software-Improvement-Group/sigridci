...
| `list_open_source_components` | Open source dependencies with risk ratings across vulnerability, freshness, legal, activity, stability, and management | `risk_dimension`: filter dimensions. `risk_min`: `NONE`, `LOW`, `MEDIUM` (default), `HIGH`, `CRITICAL`. `limit` |
