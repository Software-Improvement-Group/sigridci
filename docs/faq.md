...
Possibly. As mentioned above, Sigrid is hosted on [AWS](https://sig-sigrid-ci-upload.s3.eu-central-1.amazonaws.com/). This means your firewall needs to allow outgoing traffic in order to submit your project's source code to Sigrid. In practical terms this means the following:

- Allow outbound traffic to the [AWS IP ranges](https://ip-ranges.amazonaws.com/ip-ranges.json)
- For these IP ranges, allow outbound traffic on port 443 (Sigrid uses HTTPS exclusively)
