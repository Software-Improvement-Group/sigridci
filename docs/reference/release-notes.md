...
- **Configuration:** Sigrid administrators can now configure a custom support email address for their organization, using the "metadata" option in the menu. This address is then shown to your users when they need help, instead of the default SIG support address. See the [organization metadata documentation](../organization-integration/metadata.md#organization-level-metadata) for details.
