...
- **Sigrid CI:** The upload size checks are now more strict. If your upload exceeds the maximum size, or your upload is empty, or your upload is so small it's no longer plausible (e.g. 22 bytes), Sigrid CI will fail. This helps to catch broken pipelines or Sigrid CI configurations.
...
- **Configuration:** Previously, the Sigrid configuration required you to list all technologies in your Sigrid configuration. This had two downsides: The first is that it's cumbersome. The second is more serious: If the technologies in your system *change* over time, and you forget to update the Sigrid configuration accordingly, it means that Sigrid will not monitor the new technology. To make this more convenient and less error prone, Sigrid is now able to automatically detect the technologies in your system. This behavior is now the default for new systems. *Existing* systems and configurations are unaffected and will continue to work. Moreover, it remains possible to override the automatic detection and list the technologies manually. The [documentation for the scope configuration file](../reference/analysis-scope-configuration.md#defining-technologies) has been updated to describe both options.
...
- **On-premise:** The [AI code detection and dashboards](../capabilities/system-ai-governance.md) are now also available in on-premise Sigrid. Note that you may need an additional license in order to enable this functionality.
- **IDE integrations:** The Sigrid integrations for [Visual Studio Code](../integrations/vscode-extension.md) and [JetBrains](../integrations/jetbrains-extension.md) are now able to create issues in Azure DevOps. This functionality was already available for JIRA, but is now also available for the Azure DevOps issue tracker.
- **Integrations:** We added a script that helps you to export a SBOM for a part of your portfolio, e.g. for certain teams or divisions. You can find this script [on GitHub](https://github.com/Software-Improvement-Group/sigrid-integrations/tree/main/export-portfolio-dependencies#export-portfolio-dependencies-to-sbom).

### July 13, 2026

- **Configuration:** Sigrid administrators can now configure a custom support email address for their organization, via the "Metadata" option in the "Portfolio Settings" menu. This address is then shown to your users when they need help, instead of the default SIG support address. See the [organization metadata documentation](../organization-integration/metadata.md#organization-level-metadata) for details.
- **On-premise:** The documentation and academy links shown in the Sigrid user interface can now be configured per deployment, so they can point to a mirrored documentation site or be hidden entirely in air-gapped environments. See the [on-premise release notes](../organization-integration/onpremise-release-notes.md) for details.
...
- **Technology support:** Sigrid is now able to identify [ASP.NET core](https://dotnet.microsoft.com/en-us/apps/aspnet) framework dependencies.
- **Integrations:** We have added an integration to export Sigrid's architecture diagrams in [Graphviz format](https://graphviz.org). You can find this integration as part of the [Sigrid Integrations on GitHub](https://github.com/Software-Improvement-Group/sigrid-integrations/tree/main/architecture-export).
